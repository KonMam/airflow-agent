"""Integration tests against live Airflow 2. Requires Docker."""
...
def patch_config_for_airflow2(airflow2_env, monkeypatch, tmp_path):
    url, api_prefix = airflow2_env
    monkeypatch.setenv("AIRFLOW_URL", url)
    monkeypatch.setenv("AIRFLOW_API_VERSION", "v1")
    monkeypatch.setattr("config.config.airflow_url", url)
    monkeypatch.setattr("config.config.airflow_api_version", "v1")
...
def test_failing_task_investigated(airflow2_env, trigger_dag_and_wait):
    url, api_prefix = airflow2_env
...
def test_upstream_failure_identified(airflow2_env, trigger_dag_and_wait):
    url, api_prefix = airflow2_env
    run_id = trigger_dag_and_wait(url, api_prefix, "upstream_failure")

    upstream_answer = (
        "DIAGNOSIS: The extract task failed causing transform and load to be skipped.\n"
        "FAILING_TASK: extract\n"
        "ERROR_SUMMARY: RuntimeError in extract task.\n"
    ...
    graph = build_graph(checkpointer=MemorySaver())
    with patch(
        "agent.nodes._llm_call", return_value=_mock_llm_response(upstream_answer)
    ):
        result = graph.invoke(
            {
                "dag_id": "upstream_failure",
                "run_id": run_id,
                "airflow_version": "v1",
                "triggered_by": "test",
                "similar_past_investigations": [],
